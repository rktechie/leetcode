...
public class MergeKSortedLists {
	 ...
	 * The comparison cost will be reduced to O(logk) for every pop and insertion to priority 
	 * queue. But finding the node with the smallest value just costs O(1) time.
	 * There are N nodes in the final linked list.
	 * 
	 * Space complexity :
	 * O(n) Creating a new linked list costs O(n) space.
	 * O(k) The code applies in-place method which cost O(1) space. 
	...
	/*
	 * 1st solution: Divide and Conquer Code's complexity is also O(nlogk) where
	 * n means the total elements and k means the size of list.
	 * 
	 * So the progress of combination is like a full binary tree, from bottom to
	 * top.
	 */
